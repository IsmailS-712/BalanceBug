...
#define EXPOSURE_INIT 0x00c00
#define EXPOSURE_STEP 0x100
#define GAIN_INIT 0x23F
...
int beaconID(alt_32 id){
	if (id == 0x00524242){
		return 0x72;
	}
	else if (id == 0x00424242){
		return 0x62;
	}
	else if (id == 0x00594242){
		return 0x79;
	}
	else if (id == 0x00574242){
		return 0x77;
	}
}

void pixel_coords(alt_32 coordinates, int colour){
	int xmin, xmax, ymin, ymax;
	int xory;

	xmin = (coordinates & 0x07FF0000)>>16;
	xmax = (coordinates & 0x000007FF);
	ymin = (coordinates & 0x07FF0000)>>16;
	ymax = (coordinates & 0x000007FF);
	...
	if (xory == 2){
		distance(ymin, ymax, colour);
	}
}

void distance(int top, int bottom, int colour){
	int pixel_dis = bottom - top;
	int actual_dis = 2400/pixel_dis + 5;
	printf("%c colour is %d cm away \n", colour, actual_dis);
}
  ...
  while(1){
       ...
       while ((IORD(0x42000,EEE_IMGPROC_STATUS)>>8) & 0xff) { 	//Find out if there are words to read
           ...
           if (word == EEE_IMGPROC_MSG_START)				//Newline on message identifier
    		   printf("\n");
           	   if ((word & 0x0000FFFF) == 0x4242){
           		   colour = beaconID(word);
           		   printf("\n");
           	   }
           	   else {
           		   pixel_coords(word, colour);
           	   }
           //printf("%08x ", word);
       }
