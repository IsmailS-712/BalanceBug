...
void setup() {
 ...
 void loop() {
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    if (com_count >= 500) {  //send out data every half second.
...
void sampling(){
  ...
  if (Boost_mode == 1){
    ...
    // com_count counts to half a second and then resets. The max frequency of the sensor is 400kHz which is a period of every 2.5microseconds
    if (com_count % 50 == 0) {
      if (power > power_prev) {
